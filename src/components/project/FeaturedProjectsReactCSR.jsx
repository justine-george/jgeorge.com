	...
	useEffect(() => {
		const fetchUserCount = async () => {
			...
			try {
				...
				const userCount =
					data.users ||
					projects.filter((p) => p.title === "Tab Keeper (Featured on Web Store)")[0]?.metrics
						?.users ||
					"520";
				setTabkeeperUserCount(userCount);
				localStorage.setItem(
					"tabKeeperUserCountData",
					JSON.stringify({ count: userCount, timestamp: new Date() }),
				);
			} catch (error) {
	...
	return (
		<ul className="list-inside list-disc space-y-8">
			{projects.map((project) => (
				<li
					className="group flex list-none flex-col-reverse items-start gap-y-2 sm:flex-row sm:gap-x-4"
					key={project.title}
				>
					<a
